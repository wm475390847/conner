    ...
    protected void buildRequest(Request.Builder builder, Api api) {
        ...
        if (!partFiles.isEmpty()) {
            partFiles.forEach(partFile -> {
                String filename = FileUtil.getLastName(partFile.getFilename());
                byte[] fileBytes = partFile.getContent().getBytes(StandardCharsets.UTF_8);
                RequestBody requestBody = RequestBody.create(MediaType.parse(String.valueOf(MultipartBody.FORM)), fileBytes);
